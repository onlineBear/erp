...
public class LoginLogDomain implements ILoginLogDomain {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRES_NEW)
    public String logoutSuccess(LogoutSuccessParam param, String operUserId, Date operTime) throws Exception {
        LoginLog po = param.toLoginLog();

        po.setLoginTypeKey(LoginTypeEnum.LOGOUT.getKey());       // 登出
        po.setOperTime(operTime);
        po.setAreSuccessful(true);  // 成功
    ...
    @Override
    @Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRES_NEW)
    public String logoutFailed(LogoutFailedParam param, String operUserId, Date operTime) throws Exception {
        LoginLog po = param.toLoginLog();
        po.setLoginTypeKey(LoginTypeEnum.LOGIN.getKey());   // 登出
        po.setOperTime(operTime);
        po.setAreSuccessful(false); // 失败
